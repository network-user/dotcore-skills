# Правила проекта для агентов

> **Канон этих шаблонов - подскилл [`sync-project-rules`](../sync-project-rules/SKILL.md).** Этот файл - зеркало для автономной работы `generate-readme`, когда подскилл не установлен. Меняешь шаблоны правил - правь в `sync-project-rules/templates.md` и синкай сюда. `generate-readme` отвечает дополнительно за лицензию и `docs/portfolio-draft.md`.

Правила проекта (`AGENTS.md`, `.mdc`, `CLAUDE.md`, `GEMINI.md`) обновляй **additive: дополняй, не переписывай**. Файла нет - создай из шаблона ниже. Файл есть - сохрани авторский текст и секции, добавь только недостающие DotCore-блоки и точечно почини факты, доказуемо расходящиеся с кодом (команды, пути). Источник правды для фактов - код и манифесты; для авторских правил - сам существующий файл.

## Разделение ролей (стандарт agents.md)

| Файл | Для кого | Содержание |
|------|----------|------------|
| `README.md` | человек-разработчик | DotCore internal doc: запуск, команды, стек, архитектура |
| `AGENTS.md` | все coding-агенты | build, test, lint, conventions, запреты, указатели |
| `.cursor/rules/dotcore-project.mdc` | Cursor | краткий контекст + ссылка на AGENTS.md |
| `CLAUDE.md` | Claude Code | тонкая обёртка → AGENTS.md |
| Rule-файл агента запуска | агент, из которого запущен скилл | его нативный файл/папка (таблица ниже), создать если нет |
| `LICENSE` + футер `## Лицензия` | любой, кто открыл репо | строгий All Rights Reserved (см. [license.md](license.md)) |
| `docs/portfolio-draft.md` | DotBioSite | только если `audience=portfolio` |

README остаётся сухим техдоком. Операционные инструкции для агентов - в `AGENTS.md`, не дублируй длинные command-листы в README.

## Порядок записи (шаг 5 workflow)

1. `AGENTS.md` - канонический файл
2. **Rule-файл агента, из которого запущен скилл** - создай файл и папку, если их нет (таблица «Агент запуска» ниже)
3. `.cursor/rules/dotcore-project.mdc` - создай `.cursor/rules/` если нет
4. `CLAUDE.md` - обёртка
5. `LICENSE` + футер `## Лицензия` в README - см. [license.md](license.md)
6. `docs/portfolio-draft.md` - если `audience=portfolio`

Codex, Jules, Aider, Copilot читают корневой `AGENTS.md` нативно - отдельный файл для Codex не нужен.

## Агент запуска → файл правил

Определи, из какого агента запущен скилл (по окружению/контексту), и **обязательно** создай или обнови его нативный файл правил; если папки нет - создай её. Канонический `AGENTS.md` пишется всегда, независимо от агента.

| Агент запуска | Создать/обновить | Папка (создать, если нет) |
|---------------|------------------|---------------------------|
| Cursor | `.cursor/rules/dotcore-project.mdc` | `.cursor/rules/` |
| Claude Code | `CLAUDE.md` | корень |
| Codex / Copilot / Jules / Aider | `AGENTS.md` (нативно, уже есть) | корень |
| Gemini CLI | `GEMINI.md` (+ `.gemini/` если используется) | корень / `.gemini/` |
| OpenCode / Goose / Roo / Junie / Amp | `AGENTS.md` (нативно) | корень |

Большинство агентов читают `AGENTS.md` нативно - отдельный файл нужен только Cursor (`.mdc`), Claude (`CLAUDE.md`) и Gemini (`GEMINI.md`). Не плоди дубликаты для агентов, которым хватает `AGENTS.md`.

## README-sync (глобальные изменения)

В `AGENTS.md`, `CLAUDE.md` и `.mdc` встрой правило: **при глобальных изменениях функционала** агент параллельно обновляет README через скилл `generate-readme` (пересчёт LoC, команды, стек, архитектура) и синхронизирует `AGENTS.md` (additive).

- **Глобальные** (обновлять README): новые/удалённые команды или скрипты, новый/удалённый модуль или пакет, смена архитектуры, изменение зависимостей/стека, новая/убранная фича, смена runtime, переименование точек входа.
- **Не глобальные** (README не трогать): опечатки, внутренний рефактор без изменения команд/API, правки комментариев, мелкие багфиксы.

---

## Шаблон `AGENTS.md`

Используй шаблон, **только если `AGENTS.md` ещё нет** (тогда создай). Если файл есть - это чеклист обязательных DotCore-блоков: добавь недостающие, авторское не переписывай. Язык: **русский** (как README), техтермины и команды - как в репо. Целевой объём нового файла: **80-150 строк**, TOC-стиль, без marketing.

```markdown
# AGENTS.md

> Инструкции для AI coding agents. Человеческий обзор - в [README.md](README.md).
> Перегенерировано скиллом `generate-readme`. Источник правды - код репозитория.

## Профиль проекта

- **Тип:** {project_type}
- **Аудитория:** {audience}
- **Runtime:** {primary_runtime}
- **Монорепо:** {yes|no}

## Быстрый старт

```bash
{реальные install + dev команды, 2-4 строки}
```

{порт, URL, если есть}

## Сборка и проверки

| Действие | Команда |
|----------|---------|
| Установка | `{cmd}` |
| Dev | `{cmd}` |
| Тесты | `{cmd или «нет тестов в репо»}` |
| Lint / typecheck | `{cmd или —}` |
| Build | `{cmd или —}` |

Команды - только из `package.json` / `Makefile` / `pyproject.toml` / CI.

## Структура репозитория

{ASCII-дерево 2-3 уровня - то же что в README ## Архитектура, можно короче}

## Соглашения

- **Язык документации:** русский (если проект не EN-only).
- **Стиль кода:** {из eslint/ruff/pyproject/black, или «следуй существующим файлам»}
- **Именование:** {паттерны из репо}
- **Монорепо:** {filter-команды turbo/pnpm/npm workspaces, если есть}

## Переменные окружения

| Переменная | Назначение |
|------------|------------|
| `{NAME}` | {из явно переданного sanitized списка / docker-compose / README, без значений; `.env*` не открывать} |

Не читай `.env`. Не коммить секреты.

## Что делать агенту

- Перед правками прочитай затронутые файлы и соседний код.
- После изменений запусти релевантные тесты/lint из таблицы выше.
- **README-sync:** при глобальных изменениях функционала (новые/удалённые команды, модули, зависимости, смена архитектуры или runtime) обнови `README.md` и `AGENTS.md` через скилл `generate-readme` - в том числе пересчёт LoC. Мелкие правки (опечатки, внутренний рефактор) README не трогают.
- Не латай разметку README вручную - перегенерируй скиллом.
- Минимальный diff - не рефактори несвязанный код.
- Числа, пути, версии - только из репозитория.

## Чего не делать

- Не выдумывать команды, зависимости, env, API endpoints.
- Не добавлять `<details>`, centered hero, emoji в README DotCore.
- Не менять `docs/cover.svg` без регенерации обложки.
- Не менять `LICENSE` и текст лицензии без явного запроса пользователя.
- Не коммитить секреты, токены, `.env`.
- Не удалять маркеры `<!-- loc:start -->` / `<!-- loc:end -->` в README.

## Документация

- [README.md](README.md) - запуск, команды, стек, архитектура
- {ссылки на ARCHITECTURE.md, docs/, CONTRIBUTING - только если файлы существуют}

## DotCore

Проект следует стандарту DotCore: плоский технический README, SVG-обложка DotBioSite, LoC-бейдж. При запросе «обнови README» используй скилл `generate-readme`.
```

Удали пустые секции (например «Переменные окружения» если нет безопасного списка имён).

---

## Шаблон `.cursor/rules/dotcore-project.mdc`

Имя файла фиксировано: `dotcore-project.mdc`. При конфликте с другими rules - **обнови этот файл**, не создавай дубликат.

```markdown
---
description: DotCore контекст проекта - сборка, тесты, соглашения. Читай AGENTS.md для деталей.
globs:
  - "**/*"
alwaysApply: true
---

# DotCore Project Context

Канонические инструкции: [AGENTS.md](../../AGENTS.md) в корне репозитория.

## Кратко

- **Тип:** {project_type}
- **Установка:** `{install_cmd}`
- **Dev:** `{dev_cmd}`
- **Тесты:** `{test_cmd или нет}`

## Правила

- Источник правды - код, не устаревший README.
- README в стандарте DotCore: без marketing, без `<details>`, без centered hero.
- Обложка: {docs/cover.svg для GitHub | inline SVG для IDE}.
- LoC-бейдж - 4-й в строке header, между `<!-- loc:start -->` и `<!-- loc:end -->`.
- При глобальных изменениях (команды, модули, зависимости, архитектура) обнови README через скилл `generate-readme`.
- Минимальный scope изменений.

## README

Человеческая документация: [README.md](../../README.md). Не дублируй её целиком в чат - ссылайся на секции.

Перегенерация: скилл `generate-readme` (обновляет README + AGENTS.md + этот rule).
```

Путь `../../AGENTS.md` корректен для `.cursor/rules/dotcore-project.mdc`.

---

## Шаблон `CLAUDE.md`

Тонкая обёртка (15-25 строк). **AGENTS.md - единый источник правды.**

```markdown
# Claude Code

Прочитай [AGENTS.md](AGENTS.md) перед любой задачей в этом репозитории.

## Приоритет контекста

1. Явный запрос пользователя
2. [AGENTS.md](AGENTS.md)
3. [README.md](README.md)
4. Код в затронутых файлах

## Триггеры

- «Обнови README» / «сгенерируй README» → скилл `generate-readme`
- Глобальные изменения функционала (команды, модули, зависимости, архитектура) → обнови README + AGENTS.md тем же скиллом
- Правки документации → стандарт DotCore (см. AGENTS.md)

Не дублируй содержимое AGENTS.md здесь - обновляй AGENTS.md через скилл.
```

Если в репо уже был развёрнутый `CLAUDE.md` - **не переписывай его**: сохрани авторские секции как есть, добавь лишь ссылку на AGENTS.md как канон и недостающее правило README-sync.

---

## Шаблон `docs/portfolio-draft.md`

Только при `audience=portfolio`. Не вставляй в README.

```markdown
# Черновик для DotBioSite

> Служебный файл. Не для читателей репозитория. Копировать в `src/content/projects/{slug}.json`.

**Слоган:** {одна строка}
**Описание:** {2-3 предложения, сухо, факты}
**Факты:** {3-4 факта через ·}
**Стек:** {основные технологии через ·}

## Соответствие JSON

| Поле здесь | Ключ DotBioSite |
|------------|-----------------|
| Слоган | `tagline.ru` |
| Описание | `description.ru` |
| Факты | `highlights` |
| Стек | `stack` |
```

---

## Миграция и слияние

| Ситуация | Действие |
|----------|----------|
| Есть `.cursorrules` (legacy) | Перенеси уникальные правила в `AGENTS.md`, не копируй в README |
| Есть `CLAUDE.md` с контентом | Слей уникальное в AGENTS.md или оставь секцию «Дополнительно» в CLAUDE.md |
| Есть `AGENTS.md` устаревший | Дополни недостающими DotCore-блоками; устаревшие факты - точечно; не переписывай |
| Нет `.cursor/` | Создай `.cursor/rules/dotcore-project.mdc` |
| Monorepo с пакетами | Один AGENTS.md в корне; вложенные AGENTS.md - только если уже есть в subpackages (не создавай без запроса) |

## Чеклист project rules

- [ ] Новый `AGENTS.md` создан из шаблона, либо существующий **дополнен** (не переписан); команды проверены по манифестам.
- [ ] Rule-файл агента запуска создан; папка создана, если её не было.
- [ ] `AGENTS.md`/`CLAUDE.md`/`.mdc` содержат правило README-sync на глобальные изменения.
- [ ] `.cursor/rules/dotcore-project.mdc` существует, `alwaysApply: true`.
- [ ] `CLAUDE.md` ссылается на AGENTS.md, не дублирует его.
- [ ] `LICENSE` + футер `## Лицензия` созданы по [license.md](license.md).
- [ ] Нет секретов и значений из `.env`.
- [ ] `docs/portfolio-draft.md` создан только для portfolio-аудитории.
- [ ] Пути в mdc-файле ведут на реальные `AGENTS.md` и `README.md`.
