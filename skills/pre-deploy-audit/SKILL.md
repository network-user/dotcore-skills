...
description: >-
  ...
  провал. На PASS - цветные flat-бейджи аудита в README. Cursor,
  Claude Code, Codex. Use when просят проверить проект перед деплоем или
  публикацией, найти утечки секретов/ключей/PII, проаудитить безопасность кода.
---

# Pre-Deploy Audit

Проверка репозитория перед деплоем или публикацией: что нельзя выкатывать (утечки) и что в коде небезопасно (уязвимости). Результат - вердикт и, на PASS, цветные бейджи аудита в README.
...
6. **Stamp** (только на PASS) - впиши блок цветных flat-бейджей аудита в README по [badge.md](badge.md). Картинки нет, только бейджи.
...
- **Минимальный diff.** Аудит читает; не рефактори и не «чини» код по ходу. Правки - только бейдж в README на PASS.
...
- [ ] Бейдж выдан **только** на PASS; при провале существующий бейдж снят, не оставлен устаревший
- [ ] Правок кода нет; изменён только README (блок бейджей в маркерах) на PASS
