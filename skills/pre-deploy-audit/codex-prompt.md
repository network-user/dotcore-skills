...
4. **Validate** - свежий verifier на каждый candidate; parent пишет `findings.json` со статусами `confirmed` / `needs_validation` / `rejected`, запускает `node validate-findings.cjs`, обычную проверку ledger и перед promotion запускает `node validate-coverage-ledger.cjs --final`.
