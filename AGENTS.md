...
- **Runtime:** PowerShell 5.1+ / Bash + Python 3; Node.js нужен для zero-dependency validators полного `pre-deploy-audit`
...
- **Полный security-аудит:** `skills/pre-deploy-audit/` хранит coverage-led workflow, `findings.json`/ledger contracts и validators; промежуточные run-артефакты не считать отчётом без независимой проверки.
...
- При изменении полного аудита прогони `node skills/pre-deploy-audit/validate-findings.test.cjs` и `node skills/pre-deploy-audit/validate-coverage-ledger.test.cjs`; target-controlled код запускай только в sandbox без live-сервисов.
